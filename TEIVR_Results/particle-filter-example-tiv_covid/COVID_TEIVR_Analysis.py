...
for ii in range(0, len(patient_list)):
    try:
        ...
        param_names = ['beta', 'phi', 'rho', 'delta', 'pi', 'lnV0']

        for param in param_names:
            paramEst = pst_df[pst_df['name'] == param]
            paramEst.to_csv(f"{out_dir}/{param}Est.csv", index=False)
        ...
        parameters = ['V', 'T', 'E', 'I', 'R']
        for param in parameters:
            plt_df = state_df[state_df['unit'] == param]
            ...
            plt_df.to_csv(f"{out_dir}/{param}_plt_df.csv", index=False)
            state_p9 = state_plt_p9(plt_df, plt_df_obs)
            state_p9.save(f"{out_dir}/{param}-demo-state-trajectory.png",
